...
def runDobble(runner: "Runner", img, cnt):
    """get tensor"""
    ...
    while count < cnt:
        ...
        #    TopK(softmax, pre_output_size, "./dobble_dataset/dobble_symbols.txt")
...
def main(argv):
    ...
    for i in range(int(threadnum)):
        t1 = threading.Thread(target=runDobble, args=(all_dpu_runners[i], img, cnt))
        threadAll.append(t1)
    for x in threadAll:
        x.start()
    for x in threadAll:
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("usage : python3 dobble_benchmark.py <thread_number> <dobble_xmodel_file>")
    else:
        main(sys.argv)
