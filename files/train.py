...
'''
Training script for dobble tutorial.
'''
...
def train(input_height,input_width,input_chan,tfrec_dir,batchsize,learnrate,epochs,chkpt_dir,tboard):
    ...
    Define the model
    '''
    model = customcnn(input_shape=(input_height, input_width, input_chan),classes=58,filters=[8,16,32,64,128])
...
def run_main():
    ...
    ap.add_argument('-t', '--tfrec_dir',   type=str,   default='build/tfrecords',    help='Path of folder containing TFRecord files.')
    ap.add_argument('-b', '--batchsize',   type=int,   default=32,     help='Training batchsize. Must be an integer. Default is 32.')
    ...
    ap.add_argument('-tb','--tboard',      type=str,   default='build/tb_logs', help='path to folder for saving TensorBoard data. Default is build/tb_logs.')
    args = ap.parse_args()
